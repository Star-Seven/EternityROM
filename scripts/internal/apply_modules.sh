...
APPLY_MODULE()
{
    ...
    if ! grep -q "^SKIPUNZIP=1$" "$MODPATH/customize.sh" 2> /dev/null; then
        ...
        [ -d "$MODPATH/prism" ] && ADD_TO_WORK_DIR "$MODPATH" "prism" "." 0 2000 755 "u:object_r:vendor_configs_file:s0"
        [ -d "$MODPATH/optics" ] && ADD_TO_WORK_DIR "$MODPATH" "optics" "." 0 2000 755 "u:object_r:vendor_configs_file:s0"
    fi
